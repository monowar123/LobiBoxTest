...
//------------------------------------------------------------------------------
    ...
    var LobiboxBase = {
        ...
        _addCloseButton: function () {
            var me = this;
            var closeBtn = $('<span class="btn-close"></span>');  //&times;
...
//------------------------------------------------------------------------------
    ...
    Lobibox.base.OPTIONS = {
        ...
        icons: {
            bootstrap: {
                confirm: 'glyphicon glyphicon-none',
                success: 'glyphicon glyphicon-none',
                error: 'glyphicon glyphicon-none',
                warning: 'glyphicon glyphicon-none',
                info: 'glyphicon glyphicon-none'
            },
            fontAwesome: {
...
//            ALERT_OPTIONS = $.extend({}, LobiboxAlert.OPTIONS, Lobibox.alert.DEFAULTS);
        ...
        _init: function () {
            ...
            if (me.$options.iconClass) {
                d.append($('<div class="lobibox-icon-wrapper"></div>')
                    .append('<div class="lobibox-icon ' + me.$options.iconClass + '"></div>'))
                ;
            }
            d.append('<div class="lobibox-body-text-wrapper"><span class="lobibox-body-text">' + me.$options.msg + '</span></div>');
            me._setContent(d.html());
...
//------------------------------------------------------------------------------
        ...
        var _createNotifyWrapper = function () {
            ...
            if (true) {                        //$wrapper.length === 0
                
                if (me.$options.htmlContainar != '') {
                    $wrapper = $('<div class="html-containar"></div>')                             //change the html containar
                        .addClass(selector.replace(/\./g, ' ').trim())
                        .appendTo($('#' + me.$options.htmlContainar));

                    $('#' + me.$options.htmlContainar).css({'position':'relative'});  //make the containar div relative
                }
                else {
                    $wrapper = $('<div></div>')
                        .addClass(selector.replace(/\./g, ' ').trim())
